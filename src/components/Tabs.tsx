    ...
    return (
      <div className="flex ml-1 flex-wrap border-b border-gray-300 dark:border-gray-700">
        {tabs.map((tab) => (
          <div
            key={tab.id}
            className={`flex items-center rounded-t-md px-4 py-0 text-sm cursor-pointer ${
              activeTab === tab.id
        ...
        <button
          onClick={addTab}
          className="px-4 py-2 text-gray-600 hover:text-gray-800 rounded-full dark:text-gray-400 dark:hover:text-gray-200"
          aria-label="Add Tab"
        >
          +
        </button>
